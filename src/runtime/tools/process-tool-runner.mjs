...
export class ProcessToolRunner extends ToolRunner {
  ...
  async run(input) {
    ...
    const started = new Date().toISOString(); let before, after = null, exitCode = null, stopped = false, stdout = '', stderr = '', failure = null;
    try {
      ...
      if (!run.interrupted) {
        ...
        if (!run.interrupted) {
          ...
          if (!run.interrupted) {
            const output = await this.#attach(run, limits); stdout = output.stdout; stderr = output.stderr;
          }
        }
      }
    ...
    const outcome = executionOutcome({ exitCode, interrupted: run.interrupted, stopped, before, after });
    const result = validateToolResult({ outcome, stdout, stderr, exit_code: exitCode, started_at: started, finished_at: new Date().toISOString(),
      ...
      execution: { backend: 'docker', identity, image: this.#image, paths, writable: binding.write, network: 'none', limits }, simulation: false }, { allowReal: true });
    run.result = result; run.status = 'COMPLETED'; return result;
  }
  #attach(run, limits) {
    return new Promise((resolve, reject) => {
      const child = spawn(this.#docker, ['--context', this.#context, 'container', 'start', '--attach', `cccp-${run.identity}`], { windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
