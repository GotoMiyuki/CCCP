...
export async function recoverController({ state, workspace, tool, taskId, context }) {
  ...
  await workspace.reconcileRepository?.(bound.repository_identity, tool);
  ...
  return { id: taskId, controller: journal.controller, journal, approval: journal.approval, workspace: bound, lease,
    revision: stored.store_revision, operations, runs: new Map(data.agent_runs.map(run => [run.run_id, run.status === 'RUNNING'
      ? jsonCopy({ ...run, status: 'INTERRUPTED', accepted: false, termination_reason: 'host_restarted' }) : run])),
